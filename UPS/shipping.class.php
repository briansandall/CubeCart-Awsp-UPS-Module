...
class UPS {
	...
	private function request() {
		...
		// Merge strategy filters
		function shipIndividually(\Awsp\Ship\Package $a, \Awsp\Ship\Package $b, &$error) {
			$ai = filter_var($a->getOption('ships_individually'), FILTER_VALIDATE_BOOLEAN);
			$bi = filter_var($b->getOption('ships_individually'), FILTER_VALIDATE_BOOLEAN);
			return !($ai || $bi);
		}
		function shipByManufacturer(\Awsp\Ship\Package $a, \Awsp\Ship\Package $b, &$error) {
		...
		if (empty($this->_settings['ship_by_manufacturer'])) {
			$filter = function(\Awsp\Ship\Package $a, \Awsp\Ship\Package $b, &$error) {
				return shipIndividually($a, $b, $error);
			};
		} else {
			$filter = function(\Awsp\Ship\Package $a, \Awsp\Ship\Package $b, &$error) {
				return shipIndividually($a, $b, $error) && shipByManufacturer($a, $b, $error);
			};
		}
